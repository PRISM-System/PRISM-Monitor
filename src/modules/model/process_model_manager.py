...
from src.modules.util.util import TEST_SCENARIOS_DATA_MAPPING
...
class ProcessModelManager:
    ...
    def __init__(self, dir_name: str = "process_models"):
        """
        Args:
            dir_name: 모델 디렉토리 이름 (기본값: "process_models")
        """
        self.dir_path = Path(__file__).resolve().parent / dir_name
    ...
    def get_model_for_process(self, process_name: str) -> Tuple[Optional[keras.Model], Optional[object], Optional[Dict]]:
        ...
        # 공정별 모델 디렉토리
        process_model_dir = self.dir_path / process_name

        if not process_model_dir.exists():
            raise ValueError(f"Model directory not found for process: {process_name} (path: {process_model_dir})")

        # 메타데이터 파일 확인
        metadata_file = process_model_dir / "model_metadata.json"
...
if __name__ == "__main__":
    ...
    if 'semiconductor_cmp' in available_processes:
        print("\n3. CMP 모델 로드 테스트:")
        try:
